...
foreach (string name in studentNames)
{
    ...
    foreach (int score in studentScores)
    {
        ...
        if (gradedAssignments <= examAssignments)
        {
            ...
            if (gradedAssignments == 5)
                examScoreAverage = sumAssignmentScores/examAssignments;
        }
        else
        {
    ...
    extraCreditAverage /= extraCreditAssignments;

    currentStudentGrade = (decimal)(sumAssignmentScores) / examAssignments;

    extraCreditPTS = currentStudentGrade - examScoreAverage;
    ...
    Console.WriteLine($"{currentStudent}\t\t{examScoreAverage}\t\t{currentStudentGrade}\t{currentStudentLetterGrade}\t{extraCreditAverage} ({extraCreditPTS} pts)");
}
